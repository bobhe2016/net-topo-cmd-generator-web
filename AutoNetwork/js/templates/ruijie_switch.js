/* 锐捷交换机 RGOS — L2/L3 双模式 */
CmdTpl.register('ruijie', 'switch', function(cfg) {
  var out = [], prefix = 'GigabitEthernet0/', vlans = _rvl(cfg), layer = cfg.switchLayer || 'l3', isL3 = (layer === 'l3');
  out.push('enable');
  ...
  if (isL3) {
    ...
    vlans.forEach(function(v) {
      ...
      var ip = _findCustomIp(cfg, v) || _rgw(cfg.ip, v);
      out.push('! VLAN ' + v + ' 网关');
  ...
  _rtrunks(out, (cfg.uplinks || []).concat(cfg.peerDevices || []), prefix, vlans);
  _rtrunks(out, cfg.downlinks || [], prefix, vlans);
